...
vector<int> convert_cf(string& cantus_firmus) {
    ...
    for (size_t i = 0; i < cantus_firmus.length(); i+= chunkSize) {
        string midicentValue = cantus_firmus.substr(i, chunkSize);
        try {
            int midiValue = stoi(midicentValue, nullptr, 10) / 100;
            cf.push_back(midiValue);
        } catch (const invalid_argument& e) {
            cerr << "Invalid cantus firmus input string" << midicentValue << endl;
        }
    }
    return cf;
}